...
- modules: {module_path: {function_name: method_name}}
  The ability to provide a json schema for property names is new in Draft 6, and I'm using Draft 4
  But we can just not check that the module paths are pathlike.
  What if I want the method name to be the same as the module name?
  What if I want that usually, but not always?
- methods: {method_name: {module: module_path, function: function_name (defaults to method name)}}
  Could require repeating the same module path over and over.
- modules: just a list of module paths, as at present, which we can check
  methods: as above, check to make sure each module is listed in the modules list
  If "module" is not provided, search all of them and use the first one found.
  If "function" is not provided, it will be the method name.
- modules: just a list of paths, as at present, which we can check
  methods: {method_name: function_name}
  Doesn't allow the same function name to be defined in more than one module.
  (or, at least, not in add_methods of more than one module)
  But maybe that's a small price to pay for the simplicity of this approach.
Decision: I chose that last one.

Should we drop add_methods altogether, then, or keep it for verification?
- Drop:
  - just adds work to create it for each module
  - what if a method that wasn't intended to be addable needs to be? (really shouldn't happen)
- Keep:
  - reduces the chance of a name conflict
  - name conflicts are the tradeoff we made with the simple approach, so this mitigates that
  - the additional work is pretty minor, and only happens once per module.
Decision: keep it
