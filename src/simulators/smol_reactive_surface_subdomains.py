...
and a diffusion constant on the nonreactive inclusions,
...
class SUConditions(simulator_general.GenericConditions):
  ...
  User-Provided Attributes:
    ...
    - subdomain_D = dictionary of diffusion constant scaling factors for subdomains
  ...
  __slots__=['dirichlet','species','subdomain_D','beta','potential','reactive']
...
class SUSimulator(simulator_general.GenericSimulator):
  ...
  def __init__(self,modelparams):
    ...
    #Subdomian diffusion constants
    self.Dfunc=VaryingPropertyByCell(self.meshinfo.cells,self.conditions.subdomain_D,degree=0)
    ...
    self.dx = fem.Measure('cell',domain=self.meshinfo.mesh,subdomain_data=self.meshinfo.cells)
    ...
    for s,cbar in enumerate(cbarlist):
      if self.species[s].D is not None:
        for di,factor in self.conditions.subdomain_D.items():
          if factor > 0.0: #Ignore non-diffusive subdomains
            termname='body_s%d_d%d'%(s,di)
            ufl_form=factor*self.Dbar_dict[s]*fem.dot(fem.grad(cbar),fem.grad(vlist[s]))*self.dx(di)
            allterms.add(termname,ufl_form,bilinear=True)
...
simulatorclasses={'smol_reactive_surface_subdomains':SUSimulator}
